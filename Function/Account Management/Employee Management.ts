...
export async function AddnewEmployee({
  ...
  try {
    let AccountFindStatus = await ClientAccountModel.find({ User_id: User_id }); // Finding the employee in the database
    ...
    if (AccountFindStatus.length == 0) {
      ...
      let EmployeeAlreadyExist = EmployeeFindStatus[0].Employees.filter(
        (Employee: any) => {
          return (
