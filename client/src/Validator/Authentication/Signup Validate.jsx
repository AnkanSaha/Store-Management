...
export default async function SignupValidation(X) {
  ...
  if (X.Name === "" || X.Name === null || X.Name === undefined) {
    ...
    X.Confirm_Password === "" ||
    X.Confirm_Password === null ||
    X.Confirm_Password === undefined
  ) {
    ...
    X.Confirm_Password.length < 8 ||
    X.Confirm_Password.length > 16
  ) {
    alert("Password & Confirm Password must be 8 to 16 characters long");
    return false;
  } else if (
    X.Password.includes("@") === false &&
    X.Password.includes("#") === false &&
    X.Password.includes("$") === false &&
    X.Password.includes("%") === false &&
    X.Password.includes("&") === false &&
    ...
    X.Password.includes("/") === false &&
    X.Password.includes("|") === false &&
    X.Confirm_Password.includes("@") === false &&
    X.Confirm_Password.includes("#") === false &&
    X.Confirm_Password.includes("$") === false &&
    X.Confirm_Password.includes("%") === false &&
    X.Confirm_Password.includes("&") === false &&
    ...
    alert(
      "Password & Confirm Password must contain at least one of the special characters (@, #, $, %, &, *, !, ~, ^, (, ), -, +, =, {, }, [, ], :, ;, <, >, ?, /, |)",
    );
    return false;
  } else if (
    X.Phone === "" ||
    ...
    alert(
      `Phone Number is Required & must be 10 digits long current length is ${X.Phone.length}`,
    );
    return false;
  } else if (
    X.Address === "" ||
    ...
    X.SecurityQuestion === "" ||
    X.SecurityQuestion === null ||
    X.SecurityQuestion === undefined
  ) {
    alert("Security Question is Required");
    return false;
  } else if (
    X.SecurityAnswer === "" ||
    X.SecurityAnswer === null ||
    X.SecurityAnswer === undefined
  ) {
